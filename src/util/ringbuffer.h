...
class RingBuffer {
    public:
        ...
        void pop_blocking(size_t len) { CRITICAL_SECTION_GUARD(m_lock); pop(len); }


        void clear() { m_head = m_tail; }

        size_t size() const          { return (m_tail-m_head) & MASK; }
        size_t size_blocking() const { CRITICAL_SECTION_GUARD(m_lock); return size(); }
        bool empty() const           { return m_tail == m_head; }
        bool empty_blocking() const  { CRITICAL_SECTION_GUARD(m_lock); return empty(); }
